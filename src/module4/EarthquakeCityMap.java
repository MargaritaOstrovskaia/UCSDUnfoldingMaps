...
public class EarthquakeCityMap extends PApplet {
	...
	// code to modify is countryQuakes, where you will store the number of earthquakes per country.
	...
	public void setup() {		
		...
		// This will work whether you are online or offline
	...
	private void addKey() {	
		...
		*/
		...
		// M.O.
		fill(255, 0, 0);
		triangle(50, 135, 60, 115, 70, 135); 
		
		fill(255, 255, 255);
		ellipse(60, 185, 20, 20);
		rect(50, 235, 20, 20);

		fill(255, 255, 0);
		ellipse(60, 345, 20, 20);
		fill(0, 0, 255);
		ellipse(60, 395, 20, 20);
		fill(255, 0, 0);
		ellipse(60, 445, 20, 20);
		
		fill(0, 0, 0);
		text("City Marker", 80, 125);
		text("Land Quake", 80, 185);
		text("Ocean Quake", 80, 245);
		
		text("Size ~ Magnitude", 50, 295);
		text("Shallow", 80, 345);
		text("Intermediate", 80, 395);
		text("Deep", 80, 445);
	}
	...
	private boolean isLand(PointFeature earthquake) {
		...
		for (Marker m : countryMarkers) {
			...
			boolean inCountry = isInCountry(earthquake, m);
			if (inCountry) {
				earthquake.putProperty("country",m.getProperty("name"));//m.getId());
				return true;
			}
		}
	...
	private void printQuakes() 
	{
		...
		//   Loop over all of the countries, e.g. using for (Marker cm : countryMarkers) { ... }
		//        
		//      Inside the loop, first initialize a quake counter.
		//      Then loop through all of the earthquake markers and check to see whether (1) that marker is on land
		//     	and (2) if it is on land, that its country property matches 
		//      the name property of the country marker.   If so, increment the country's counter.
		...
		for(Marker quake : quakeMarkers) {
			if(quake instanceof EarthquakeMarker) {
				boolean isOnLand = ((EarthquakeMarker)quake).isOnLand();
				
				if(isOnLand) {
					String name = quake.getProperty("country").toString();
		...
		System.out.println(String.format("%s : %d", "OCEAN QUAKES", oceanQuakes));
